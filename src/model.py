...
# class GlobalGeneralizedMeanPooling2D(tf.keras.layers.Layer):
#     """Implementation only works for channels-last"""
#     def __init__(self):
#         super(GlobalGeneralizedMeanPooling2D, self).__init__()
#
#     def build(self, input_shape):
#         self.p = tf.Variable(initial_value=1., trainable=True, dtype='float16')
#
#     def call(self, inputs):
#         x = tf.nn.relu(inputs)
#         return tf.math.pow(tf.reduce_mean(
#             tf.math.pow(x, self.p), axis=[1, 2]), 1./self.p)


class NeuralNet(tf.keras.Model):

    def __init__(self, engine, input_shape,
                 units, dropout, activation, weights=None):
        ...
        for drop, unit, actv in zip(dropout, units, activation):
            if actv != None:
                ...
                self.head.add(tf.keras.layers.Dense(unit))
                self.head.add(tf.keras.layers.Activation(actv))
            else:
                self.head.add(tf.keras.layers.Dropout(drop))
